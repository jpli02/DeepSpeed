...
## We modify the MOELayer to remove 
## gating + all-to-all for Single GPU Micro-benchmarks.
class MOELayer(Base):
...
def run_deepspeed_moe_layer(top_k, exp_num, bs, seq_len, hid_dim, moe_dim, use_tutel=False):

    x = torch.randn((bs, seq_len, hid_dim), 
                dtype=torch.float16, device="cuda" if torch.cuda.is_available() else "cpu", requires_grad=True)
    expert = torch.nn.Sequential(torch.nn.Linear(hid_dim, moe_dim, dtype=torch.float16), torch.nn.Linear(moe_dim, hid_dim, dtype=torch.float16)).to(0)
    experts = Experts(expert, exp_num, 'temp_name')

    gating_func = TopKGate(hid_dim, exp_num, top_k, 1.2, 1.2, drop_tokens=True)

    moe = MOELayer(gating_func, experts, 'temp_group', 1, exp_num)
    a = moe(x)

    grads = torch.randn_like(a, device="cuda")

    print(f'topk: {top_k}, num_experts: {exp_num}, num_tokens: {seq_len}')
    torch.cuda.synchronize()
    torch.cuda.reset_peak_memory_stats()
    for _ in range(10):
        out = moe(x)
        out.backward(grads, retain_graph=True)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--ffn_dim", type=int, required=True, help="Hidden dimension")
    # parser.add_argument("--use_tutel", type=bool, required=False, help="Hidden dimension")
    # 
    # args = parser.parse_args()

    #print(f"Arguments: {args}")
    run_deepspeed_moe_layer(1, 4, 1, 4096, 7168, 2048, False)
